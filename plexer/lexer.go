...
func (r *Lexer) SkipTo(c byte) {
	r.pos += bytes.IndexByte(r.Data[r.pos:], c)
	r.consume()
}
...
func (r *Lexer) Interface() interface{} {
	...
	var isMap bool
	var isSlice bool
	if r.token.delimValue == 'O' {
		isMap = true
	} else if r.token.delimValue == 'a' {
		first := bytes.IndexByte(r.Data[r.pos:], '{')
		if r.Data[r.pos+first+1] == 's' {
			isMap = true
		} else {
			isSlice = true
		}
