...
import (
	...
	"github.com/golyakov/easyphpserialize"
)
...
func (g *Generator) genTypeDecoder(t reflect.Type, out string, tags fieldTags, indent int) error {
	...
	unmarshalerIface := reflect.TypeOf((*easyphpserialize.Unmarshaler)(nil)).Elem()
	if reflect.PtrTo(t).Implements(unmarshalerIface) {
		fmt.Fprintln(g.out, ws+"("+out+").UnmarshalEasyPHPSerialize(in)")
		return nil
	}
...
func hasCustomUnmarshaler(t reflect.Type) bool {
	t = reflect.PtrTo(t)
	return t.Implements(reflect.TypeOf((*easyphpserialize.Unmarshaler)(nil)).Elem()) ||
		t.Implements(reflect.TypeOf((*encoding.TextUnmarshaler)(nil)).Elem())
}

func hasUnknownsUnmarshaler(t reflect.Type) bool {
	t = reflect.PtrTo(t)
	return t.Implements(reflect.TypeOf((*easyphpserialize.UnknownsUnmarshaler)(nil)).Elem())
}

func hasUnknownsMarshaler(t reflect.Type) bool {
	t = reflect.PtrTo(t)
	return t.Implements(reflect.TypeOf((*easyphpserialize.UnknownsMarshaler)(nil)).Elem())
}
...
func (g *Generator) genTypeDecoderNoCheck(t reflect.Type, out string, tags fieldTags, indent int) error {
	...
	case reflect.Slice:
		...
		if elem.Kind() == reflect.Uint8 && elem.Name() == "uint8" {
			...
			fmt.Fprintln(g.out, ws+"  "+out+" = nil")
			fmt.Fprintln(g.out, ws+"} else {")
			fmt.Fprintln(g.out, ws+"  in.SkipTo('{')")
			...
			fmt.Fprintln(g.out, ws+"    in.WantSemicolon()")
			fmt.Fprintln(g.out, ws+"  }")
			fmt.Fprintln(g.out, ws+"  in.Delim('}')")
			fmt.Fprintln(g.out, ws+"}")
		}
	...
	case reflect.Interface:
		if t.NumMethod() != 0 {
			if g.interfaceIsEasyUnmarshaller(t) {
				fmt.Fprintln(g.out, ws+out+".UnmarshalEasyPHPSerialize(in)")
			} else {
				return fmt.Errorf("interface type %v not supported: only interface{} and easyphpserialize Unmarshaler are allowed", t)
			}
		} else {
			fmt.Fprintln(g.out, ws+"if m, ok := "+out+".(easyphpserialize.Unmarshaler); ok {")
			fmt.Fprintln(g.out, ws+"m.UnmarshalEasyPHPSerialize(in)")
			fmt.Fprintln(g.out, ws+"} else {")
...
func (g *Generator) interfaceIsEasyUnmarshaller(t reflect.Type) bool {
	return t.Implements(reflect.TypeOf((*easyphpserialize.Unmarshaler)(nil)).Elem())
}
...
func (g *Generator) genSliceArrayDecoder(t reflect.Type) error {
	...
	fmt.Fprintln(g.out, "func "+fname+"(in *plexer.Lexer, out *"+typ+") {")
...
func (g *Generator) genStructDecoder(t reflect.Type) error {
	...
	fmt.Fprintln(g.out, "    in.WantSemicolon()")
	fmt.Fprintln(g.out, "    if in.IsNull() {")
	fmt.Fprintln(g.out, "       in.Skip()")
	fmt.Fprintln(g.out, "       in.WantSemicolon()")
	...
	if g.disallowUnknownFields {
		fmt.Fprintln(g.out, `      in.AddError(&plexer.LexerError{
          Offset: in.GetPos(),
...
func (g *Generator) genStructUnmarshaler(t reflect.Type) error {
	...
	if !g.noStdMarshalers {
		fmt.Fprintln(g.out, "// UnmarshalPHPSerialize supports easyphpserialize.Unmarshaler interface")
		fmt.Fprintln(g.out, "func (v *"+typ+") UnmarshalPHPSerialize(data []byte) error {")
		fmt.Fprintln(g.out, "  r := plexer.Lexer{Data: data}")
		fmt.Fprintln(g.out, "  "+fname+"(&r, v)")
		fmt.Fprintln(g.out, "  return r.Error()")
		fmt.Fprintln(g.out, "}")
	}

	fmt.Fprintln(g.out, "// UnmarshalEasyPHPSerialize supports easyphpserialize.Unmarshaler interface")
	fmt.Fprintln(g.out, "func (v *"+typ+") UnmarshalEasyPHPSerialize(l *plexer.Lexer) {")
