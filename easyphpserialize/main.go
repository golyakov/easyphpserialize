...
import (
	...
	"github.com/golyakov/easyphpserialize/bootstrap"
	...
	_ "github.com/golyakov/easyphpserialize/gen"
	"github.com/golyakov/easyphpserialize/parser"
)
...
var noStdMarshalers = flag.Bool("no_std_marshalers", false, "don't generate MarshalPhpSerialize/UnmarshalPhpSerialize funcs")
...
var disallowUnknownFields = flag.Bool("disallow_unknown_fields", false, "return error if any unknown field in PHPSerialize appeared")
...
var fileNameSuffix = "_easyphpserialize.go"
...
func generate(fname string) (err error) {
	...
	var outName string
	if fInfo.IsDir() {
		outName = filepath.Join(fname, p.PkgName+fileNameSuffix)
	} else {
		if s := strings.TrimSuffix(fname, ".go"); s == fname {
			return errors.New("Filename must end in '.go'")
		} else {
			outName = s + fileNameSuffix
		}
	}
